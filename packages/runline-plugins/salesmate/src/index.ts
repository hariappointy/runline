...
function getConn(ctx: { connection: { config: Record<string, unknown> } }) {
  return { sessionToken: ctx.connection.config.sessionToken as string, linkname: ctx.connection.config.linkname as string };
}
...
async function api(conn: ReturnType<typeof getConn>, method: string, endpoint: string, body?: Record<string, unknown>, qs?: Record<string, unknown>): Promise<unknown> {
  ...
  const init: RequestInit = { method, headers: { sessionToken: conn.sessionToken, "x-linkname": conn.linkname, "Content-Type": "application/json" } };
  if (body && Object.keys(body).length > 0) init.body = JSON.stringify(body);
  const res = await fetch(url.toString(), init);
  if (!res.ok) throw new Error(`Salesmate error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
function registerCrud(
  ...
  rl.registerAction(`${resource}.create`, { description: `Create a ${resource}`, inputSchema: createSchema,
    async execute(input, ctx) {
      const data = (await api(conn(ctx), "POST", `/v1/${plural}`, input as Record<string, unknown>)) as Record<string, unknown>;
      return data.Data;
    } });

  rl.registerAction(`${resource}.get`, { description: `Get a ${resource} by ID`, inputSchema: { id: { type: "string", required: true } },
    async execute(input, ctx) {
      const data = (await api(conn(ctx), "GET", `/v1/${plural}/${(input as Record<string, unknown>).id}`)) as Record<string, unknown>;
      return data.Data;
    } });
  ...
  rl.registerAction(`${resource}.list`, { description: `Search/list ${plural}`, inputSchema: { limit: { type: "number", required: false }, fields: { type: "object", required: false, description: "Array of field names to return" }, query: { type: "object", required: false, description: "Search query object" } },
    async execute(input, ctx) {
      ...
      const body: Record<string, unknown> = { fields: p.fields ?? ["name", "id"], query: p.query ?? {} };
      const qs: Record<string, unknown> = {};
      if (p.limit) qs.rows = p.limit;
      const data = (await api(conn(ctx), "POST", `/v2/${plural}/search`, body, qs)) as Record<string, unknown>;
  ...
  rl.registerAction(`${resource}.update`, { description: `Update a ${resource}`, inputSchema: { id: { type: "string", required: true }, data: { type: "object", required: true } },
    async execute(input, ctx) {
      const p = input as Record<string, unknown>;
      const data = (await api(conn(ctx), "PUT", `/v1/${plural}/${p.id}`, p.data as Record<string, unknown>)) as Record<string, unknown>;
      return data.Data;
    } });

  rl.registerAction(`${resource}.delete`, { description: `Delete a ${resource}`, inputSchema: { id: { type: "string", required: true } },
    async execute(input, ctx) { return api(conn(ctx), "DELETE", `/v1/${plural}/${(input as Record<string, unknown>).id}`); } });
}
