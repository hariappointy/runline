...
export default function brevo(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("contact.create", {
    ...
    inputSchema: {
      ...
      attributes: {
        type: "object",
        required: false,
        description: "Contact attributes as key-value pairs",
      },
      listIds: {
        type: "array",
        required: false,
    ...
    async execute(input, ctx) {
      const { email, attributes, listIds } = (input ?? {}) as Record<
        string,
        unknown
      >;
  ...
  rl.registerAction("contact.get", {
    ...
    inputSchema: {
      identifier: {
        type: "string",
        required: true,
        description: "Email or contact ID",
      },
    },
    async execute(input, ctx) {
  ...
  rl.registerAction("contact.list", {
    ...
    async execute(input, ctx) {
      const { limit, offset, sort, modifiedSince } = (input ?? {}) as Record<
        string,
        unknown
      >;
  ...
  rl.registerAction("contact.update", {
    ...
    async execute(input, ctx) {
      const { identifier, attributes, listIds, unlinkListIds } =
  ...
  rl.registerAction("attribute.create", {
    ...
    inputSchema: {
      ...
      type: {
        type: "string",
        required: false,
        description: "Type (for normal): text, date, float, boolean",
      },
      value: {
        type: "string",
        required: false,
        description: "Value (for calculated/global)",
      },
      enumeration: {
        type: "array",
        required: false,
        description: "Array of {value, label} for category type",
      },
    },
    async execute(input, ctx) {
      const { category, name, type, value, enumeration } = input as Record<
        string,
        unknown
      >;
      const body: Record<string, unknown> = {};
  ...
  rl.registerAction("attribute.update", {
    ...
    inputSchema: {
      category: {
        type: "string",
        required: true,
        description: "Category: calculated, category, global",
      },
      name: { type: "string", required: true, description: "Attribute name" },
    ...
    async execute(input, ctx) {
      const { category, name, value, enumeration } = input as Record<
        string,
        unknown
      >;
      const body: Record<string, unknown> = {};
  ...
  rl.registerAction("email.send", {
    ...
    inputSchema: {
      ...
      to: {
        type: "array",
        required: true,
        description: "Array of recipient emails (or {email, name} objects)",
      },
    ...
    async execute(input, ctx) {
      ...
      if (cc)
        body.cc = (cc as unknown[]).map((r) =>
          typeof r === "string" ? { email: r } : r,
        );
      if (bcc)
        body.bcc = (bcc as unknown[]).map((r) =>
          typeof r === "string" ? { email: r } : r,
        );
