...
function getConn(ctx: { connection: { config: Record<string, unknown> } }) {
  ...
  return {
    instanceUrl: (c.instanceUrl as string).replace(/\/$/, ""),
...
async function api(
  ...
  const url = new URL(
    `${conn.instanceUrl}/services/data/${API_VERSION}${endpoint}`,
  );
  if (qs) {
  ...
  rl.registerAction(`${lower}.get`, {
    ...
    async execute(input, ctx) {
      return api(
        conn(ctx),
        "GET",
        `/sobjects/${sObject}/${(input as Record<string, unknown>).id}`,
      );
    },
  });
  ...
  rl.registerAction("soql.query", {
    ...
    async execute(input, ctx) {
      const data = (await api(getConn(ctx), "GET", "/query", undefined, {
        q: (input as Record<string, unknown>).query,
      })) as Record<string, unknown>;
      return data.records;
    },
  });
  ...
  rl.registerAction("sobject.describe", {
    ...
    async execute(input, ctx) {
      return api(
        getConn(ctx),
        "GET",
        `/sobjects/${(input as Record<string, unknown>).sObject}/describe`,
      );
    },
  });
}
