...
export default function hackernews(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("article.get", {
    ...
    async execute(input) {
      return firebaseRequest(
        `item/${(input as { articleId: string }).articleId}`,
      );
    },
  });
  ...
  rl.registerAction("article.search", {
    ...
    inputSchema: {
      ...
      tags: {
        ...
        required: false,
        description:
          "Filter tags: story, comment, ask_hn, show_hn, poll, front_page",
      },
    ...
    async execute(input) {
      ...
      const data = (await algoliaRequest("search", qs)) as Record<
        string,
        unknown
      >;
      return data.hits;
  ...
  rl.registerAction("user.get", {
    ...
    async execute(input) {
      return firebaseRequest(
        `user/${(input as { username: string }).username}`,
      );
    },
  });
  ...
  rl.registerAction("all.top", {
    ...
    async execute(input) {
      ...
      const items = await Promise.all(
        ids.slice(0, limit).map((id) => firebaseRequest(`item/${id}`)),
      );
      return items;
    },
  });
