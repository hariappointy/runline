...
async function api(token: string, method: string, endpoint: string, body?: Record<string, unknown>, qs?: Record<string, unknown>, fullOutput = false): Promise<unknown> {
...
async function paginate(token: string, endpoint: string, qs: Record<string, unknown> = {}, limit?: number): Promise<unknown[]> {
  const results: unknown[] = [];
  let nextToken: string | undefined;
  qs.max_results = limit && limit < 100 ? limit : 10;
  do {
    if (nextToken) qs.next_token = nextToken;
    const res = await api(token, "GET", endpoint, undefined, { ...qs }, true) as Record<string, unknown>;
    const data = res.data as unknown[] | undefined;
    if (data) results.push(...data);
    nextToken = (res.meta as Record<string, unknown>)?.next_token as string | undefined;
...
export default function twitter(rl: RunlinePluginAPI) {
  ...
  const t = (ctx: { connection: { config: Record<string, unknown> } }) => ctx.connection.config.bearerToken as string;
  ...
  rl.registerAction("tweet.create", { description: "Create a tweet (post, quote, or reply)",
    ...
    async execute(input, ctx) {
      ...
      if (p.replyToTweetId) body.reply = { in_reply_to_tweet_id: p.replyToTweetId };
  ...
  rl.registerAction("tweet.delete", { description: "Delete a tweet", inputSchema: { id: { type: "string", required: true } },
    async execute(input, ctx) { return api(t(ctx), "DELETE", `/tweets/${(input as Record<string, unknown>).id}`); } });

  rl.registerAction("tweet.like", { description: "Like a tweet (requires user context)",
    inputSchema: { tweetId: { type: "string", required: true } },
    async execute(input, ctx) {
      const user = await api(t(ctx), "GET", "/users/me") as Record<string, unknown>;
      return api(t(ctx), "POST", `/users/${user.id}/likes`, { tweet_id: (input as Record<string, unknown>).tweetId });
    } });
  ...
  rl.registerAction("tweet.retweet", { description: "Retweet a tweet (requires user context)",
    ...
    async execute(input, ctx) {
      const user = await api(t(ctx), "GET", "/users/me") as Record<string, unknown>;
      return api(t(ctx), "POST", `/users/${user.id}/retweets`, { tweet_id: (input as Record<string, unknown>).tweetId });
    } });
  ...
  rl.registerAction("tweet.search", { description: "Search recent tweets (last 7 days)",
    ...
    async execute(input, ctx) {
      ...
      if (limit) { return paginate(t(ctx), "/tweets/search/recent", qs, limit); }
