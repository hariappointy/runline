...
async function apiRequest(apiKey: string, endpoint: string, qs?: Record<string, unknown>): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Hunter API error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export default function hunter(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("domainSearch", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(ctx.connection.config.apiKey as string, "/domain-search", qs)) as Record<string, unknown>;
      return data.data;
    },
  });
  ...
  rl.registerAction("emailFinder", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(ctx.connection.config.apiKey as string, "/email-finder", { domain, first_name: firstName, last_name: lastName })) as Record<string, unknown>;
      return data.data;
    },
  });
  ...
  rl.registerAction("emailVerifier", {
    ...
    async execute(input, ctx) {
      const data = (await apiRequest(ctx.connection.config.apiKey as string, "/email-verifier", { email: (input as { email: string }).email })) as Record<string, unknown>;
      return data.data;
    },
  });
}
