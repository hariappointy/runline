...
export default function linear(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("issue.get", {
    ...
    async execute(input, ctx) {
      const data = await gql(
        key(ctx),
        `query($id: String!) { issue(id: $id) { id identifier title description url priority state { id name } assignee { id name } labels { nodes { id name } } createdAt updatedAt } }`,
  ...
  rl.registerAction("issue.list", {
    ...
    async execute(input, ctx) {
      ...
      const data = await gql(
        key(ctx),
        `query { issues(first: ${limit}${filter}) { nodes { id identifier title url priority state { name } assignee { name } createdAt } } }`,
      );
  ...
  rl.registerAction("issue.addLink", {
    ...
    async execute(input, ctx) {
      const { issueId, relatedIssueId, type } = input as Record<
        string,
        unknown
      >;
      const data = await gql(
