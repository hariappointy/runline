...
export default function pushcut(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("notification.send", {
    ...
    async execute(input, ctx) {
      ...
      const res = await fetch(`${BASE}/notifications/${encodeURIComponent(p.notificationName as string)}`, {
        method: "POST",
        headers: { "API-Key": key(ctx), "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      if (!res.ok) throw new Error(`Pushcut error ${res.status}: ${await res.text()}`);
      return res.json();
    },
  });
}
