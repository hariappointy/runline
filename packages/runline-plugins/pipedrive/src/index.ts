...
async function api(token: string, method: string, endpoint: string, body?: Record<string, unknown>, qs?: Record<string, unknown>, version: "v1" | "v2" = "v2"): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Pipedrive error ${res.status}: ${await res.text()}`);
  const json = await res.json() as Record<string, unknown>;
  if (json.success === false) throw new Error(`Pipedrive: ${JSON.stringify(json)}`);
  return json.data ?? json;
}

async function paginate(token: string, endpoint: string, qs: Record<string, unknown> = {}): Promise<unknown[]> {
  ...
  do {
    ...
    cursor = (json.additional_data as Record<string, unknown> | undefined)?.next_cursor as string | undefined;
    if (!cursor) break;
  } while (true);
  return results;
}

function registerCrud(rl: RunlinePluginAPI, resource: string, endpoint: string, opts?: { hasSearch?: boolean; hasDuplicate?: boolean; updateMethod?: string; extraCreate?: Record<string, unknown>; extraUpdate?: Record<string, unknown> }) {
  ...
  rl.registerAction(`${resource}.create`, { description: `Create a ${resource}`,
    ...
    async execute(input, ctx) { const t = getConn(ctx); return api(t, "POST", endpoint, (input as Record<string, unknown>).data as Record<string, unknown>); } });

  rl.registerAction(`${resource}.get`, { description: `Get a ${resource}`, inputSchema: { id: { type: "number", required: true } },
    async execute(input, ctx) { const t = getConn(ctx); return api(t, "GET", `${endpoint}/${(input as Record<string, unknown>).id}`); } });
  ...
  rl.registerAction(`${resource}.list`, { description: `List ${resource}s`,
    ...
    async execute(input, ctx) {
      ...
      if (p.limit) { qs.limit = p.limit; return api(t, "GET", endpoint, undefined, qs); }
  ...
  rl.registerAction(`${resource}.update`, { description: `Update a ${resource}`,
    ...
    async execute(input, ctx) { const t = getConn(ctx); const p = input as Record<string, unknown>; return api(t, updateMethod, `${endpoint}/${p.id}`, p.data as Record<string, unknown>); } });

  rl.registerAction(`${resource}.delete`, { description: `Delete a ${resource}`, inputSchema: { id: { type: "number", required: true } },
    async execute(input, ctx) { const t = getConn(ctx); await api(t, "DELETE", `${endpoint}/${(input as Record<string, unknown>).id}`); return { success: true }; } });
  ...
  if (opts?.hasSearch) {
    rl.registerAction(`${resource}.search`, { description: `Search ${resource}s`,
      ...
      async execute(input, ctx) {
        ...
        // Search uses v1 API
        const res = await api(t, "GET", `${endpoint}/search`, undefined, qs, "v1") as Record<string, unknown>;
        if (Array.isArray(res)) return (res as Array<Record<string, unknown>>).map((r) => r.item ?? r);
        return res;
      } });
  }
  ...
  if (opts?.hasDuplicate) {
    rl.registerAction(`${resource}.duplicate`, { description: `Duplicate a ${resource}`, inputSchema: { id: { type: "number", required: true } },
      async execute(input, ctx) { const t = getConn(ctx); return api(t, "POST", `${endpoint}/${(input as Record<string, unknown>).id}/duplicate`); } });
  }
}
...
export default function pipedrive(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("dealProduct.update", { description: "Update a product in a deal",
    ...
    async execute(input, ctx) {
      const t = getConn(ctx);
      const p = input as Record<string, unknown>;
      return api(t, "PATCH", `/deals/${p.dealId}/products/${p.productAttachmentId}`, p.data as Record<string, unknown>);
    } });
  ...
  rl.registerAction("file.get", { description: "Get file metadata", inputSchema: { id: { type: "number", required: true } },
    async execute(input, ctx) { return api(getConn(ctx), "GET", `/files/${(input as Record<string, unknown>).id}`, undefined, undefined, "v1"); } });

  rl.registerAction("file.delete", { description: "Delete a file", inputSchema: { id: { type: "number", required: true } },
    async execute(input, ctx) { await api(getConn(ctx), "DELETE", `/files/${(input as Record<string, unknown>).id}`, undefined, undefined, "v1"); return { success: true }; } });
