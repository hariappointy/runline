...
async function getAccessToken(conn: { base: string; clientId: string; secret: string }): Promise<string> {
...
export default function paypal(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("payout.create", {
    ...
    async execute(input, ctx) {
      ...
      const header: Record<string, unknown> = { sender_batch_id: p.senderBatchId };
  ...
  rl.registerAction("payout.get", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(getConn(ctx), "GET", `/payments/payouts/${p.payoutBatchId}`, undefined, qs)) as Record<string, unknown>;
      return data.items;
    },
  });
