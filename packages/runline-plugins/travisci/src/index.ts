...
async function apiRequest(
  ...
  if (!res.ok) throw new Error(`TravisCI error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export default function travisci(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("build.get", {
    ...
    async execute(input, ctx) { return apiRequest(key(ctx), "GET", `/build/${(input as Record<string, unknown>).buildId}`); },
  });
  ...
  rl.registerAction("build.list", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(key(ctx), "GET", "/builds", undefined, qs)) as Record<string, unknown>;
      return data.builds;
    },
  });
  ...
  rl.registerAction("build.cancel", {
    ...
    inputSchema: { buildId: { type: "string", required: true } },
    async execute(input, ctx) { return apiRequest(key(ctx), "POST", `/build/${(input as Record<string, unknown>).buildId}/cancel`); },
  });

  rl.registerAction("build.restart", {
    description: "Restart a build",
    inputSchema: { buildId: { type: "string", required: true } },
    async execute(input, ctx) { return apiRequest(key(ctx), "POST", `/build/${(input as Record<string, unknown>).buildId}/restart`); },
  });
