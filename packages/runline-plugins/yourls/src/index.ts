...
function getConn(ctx: { connection: { config: Record<string, unknown> } }) {
  ...
  return { url: (c.url as string).replace(/\/$/, ""), signature: c.signature as string };
}

async function apiRequest(conn: { url: string; signature: string }, qs: Record<string, unknown>): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Yourls error ${res.status}: ${await res.text()}`);
...
export default function yourls(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("url.expand", {
    ...
    async execute(input, ctx) {
      return apiRequest(getConn(ctx), { action: "expand", shorturl: (input as Record<string, unknown>).shortUrl });
    },
  });
  ...
  rl.registerAction("url.stats", {
    ...
    inputSchema: { shortUrl: { type: "string", required: true } },
    async execute(input, ctx) {
      const data = (await apiRequest(getConn(ctx), { action: "url-stats", shorturl: (input as Record<string, unknown>).shortUrl })) as Record<string, unknown>;
      return data.link;
    },
  });
}
