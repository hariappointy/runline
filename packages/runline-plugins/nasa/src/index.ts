...
async function apiRequest(
  apiKey: string, endpoint: string, qs: Record<string, unknown> = {},
): Promise<unknown> {
  const url = new URL(`${BASE_URL}${endpoint}`);
  url.searchParams.set("api_key", apiKey);
  for (const [k, v] of Object.entries(qs)) { if (v !== undefined && v !== null) url.searchParams.set(k, String(v)); }
  const res = await fetch(url.toString());
  if (!res.ok) throw new Error(`NASA API error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export default function nasa(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("apod.get", {
    ...
    async execute(input, ctx) {
      const qs: Record<string, unknown> = { date: formatDate((input as Record<string, unknown>)?.date) ?? today() };
  ...
  rl.registerAction("asteroidNeoFeed.get", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(key(ctx), "/neo/rest/v1/feed", qs)) as Record<string, unknown>;
  ...
  rl.registerAction("asteroidNeoLookup.get", {
    ...
    async execute(input, ctx) {
      const { asteroidId, includeCloseApproachData } = input as Record<string, unknown>;
      const data = (await apiRequest(key(ctx), `/neo/rest/v1/neo/${asteroidId}`)) as Record<string, unknown>;
  ...
  rl.registerAction("asteroidNeoBrowse.list", {
    ...
    async execute(input, ctx) {
      const qs: Record<string, unknown> = {};
      if ((input as Record<string, unknown>)?.limit) qs.size = (input as Record<string, unknown>).limit;
      const data = (await apiRequest(key(ctx), "/neo/rest/v1/neo/browse", qs)) as Record<string, unknown>;
      return data.near_earth_objects;
    },
  });
  ...
  const donkiEndpoints: Array<{ name: string; path: string; description: string }> = [
