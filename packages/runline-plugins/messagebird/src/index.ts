...
async function apiRequest(
  accessKey: string, method: string, endpoint: string,
  ...
  if (!res.ok) throw new Error(`MessageBird API error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export default function messagebird(rl: RunlinePluginAPI) {
  ...
  const key = (ctx: { connection: { config: Record<string, unknown> } }) => ctx.connection.config.accessKey as string;
  ...
  rl.registerAction("sms.send", {
    ...
    async execute(input, ctx) {
      ...
      const body: Record<string, unknown> = {
        ...
        body: p.body,
        recipients: (p.recipients as string).split(",").map((r) => Number.parseInt(r.trim(), 10)),
      };
