...
async function jsonRpc(url: string, params: unknown): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Odoo HTTP error ${res.status}: ${await res.text()}`);
...
async function login(url: string, db: string, username: string, password: string): Promise<number> {
  const uid = (await jsonRpc(url, { service: "common", method: "login", args: [db, username, password] })) as number;
...
export default function odoo(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("record.create", {
    ...
    async execute(input, ctx) {
      ...
      const id = await execute(s.url, s.db, s.uid, s.password, resolveModel(model as string), "create", fields);
      return { id };
    },
  });
  ...
  rl.registerAction("record.get", {
    ...
    async execute(input, ctx) {
      ...
      return execute(s.url, s.db, s.uid, s.password, resolveModel(p.model as string), "read", [p.id], fieldsToRead);
    },
  });
  ...
  rl.registerAction("record.list", {
    ...
    async execute(input, ctx) {
      ...
      return execute(s.url, s.db, s.uid, s.password, resolveModel(p.model as string), "search_read", filters, fields, offset, limit);
    },
  });
  ...
  rl.registerAction("record.update", {
    ...
    async execute(input, ctx) {
      const p = input as Record<string, unknown>;
      const s = await getSession(ctx);
      await execute(s.url, s.db, s.uid, s.password, resolveModel(p.model as string), "write", [p.id], p.fields);
      return { id: p.id };
    },
  });
  ...
  rl.registerAction("record.delete", {
    ...
    async execute(input, ctx) {
      const p = input as Record<string, unknown>;
      const s = await getSession(ctx);
      await execute(s.url, s.db, s.uid, s.password, resolveModel(p.model as string), "unlink", [p.id]);
      return { success: true };
    },
  });
  ...
  rl.registerAction("model.getFields", {
    ...
    async execute(input, ctx) {
      ...
      const s = await getSession(ctx);
      return execute(s.url, s.db, s.uid, s.password, resolveModel(model as string), "fields_get", [], ["string", "type", "help", "required", "name"]);
    },
  });
}
