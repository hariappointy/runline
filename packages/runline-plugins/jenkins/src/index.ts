...
export default function jenkins(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("job.copy", {
    ...
    async execute(input, ctx) {
      const { fromJob, newName } = input as {
        fromJob: string;
        newName: string;
      };
      await jk(ctx, "POST", "/createItem", undefined, undefined, {
        name: newName,
        mode: "copy",
        from: fromJob,
      });
  ...
  rl.registerAction("job.create", {
    ...
    inputSchema: {
      ...
      xml: {
        type: "string",
        required: true,
        description: "Jenkins job config XML",
      },
    },
    async execute(input, ctx) {
