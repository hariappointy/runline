...
async function paginateAll(
  ...
  const size = 100;
  while (true) {
    const data = (await apiRequest(host, token, endpoint, { ...qs, skip, limit: size })) as Record<string, unknown>;
...
function getConn(ctx: { connection: { config: Record<string, unknown> } }) {
  ...
  return {
    ...
    token: (isPreview ? cfg.previewAccessToken : cfg.deliveryAccessToken) as string,
...
export default function contentful(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("entry.list", {
    ...
    async execute(input, ctx) {
      const { environmentId, contentType, query, select, order, limit } = (input ?? {}) as Record<string, unknown>;
      const { host, token, spaceId } = getConn(ctx);
      ...
      return paginateAll(host, token, `/spaces/${spaceId}/environments/${environmentId}/entries`, qs, limit as number | undefined);
    },
  });
  ...
  rl.registerAction("asset.list", {
    ...
    async execute(input, ctx) {
      ...
      const { host, token, spaceId } = getConn(ctx);
      return paginateAll(host, token, `/spaces/${spaceId}/environments/${environmentId}/assets`, undefined, limit as number | undefined);
    },
  });
  ...
  rl.registerAction("locale.list", {
    ...
    async execute(input, ctx) {
      const { environmentId, limit } = (input ?? {}) as Record<string, unknown>;
      const { host, token, spaceId } = getConn(ctx);
      return paginateAll(host, token, `/spaces/${spaceId}/environments/${environmentId}/locales`, undefined, limit as number | undefined);
    },
  });
}
