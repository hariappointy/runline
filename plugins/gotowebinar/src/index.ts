...
async function apiRequest(
  ...
  if (!res.ok) throw new Error(`GoToWebinar API error ${res.status}: ${await res.text()}`);
...
export default function gotowebinar(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("webinar.create", {
    ...
    async execute(input, ctx) {
      const { subject, times, description: desc, timeZone, type, isPasswordProtected } = input as Record<string, unknown>;
  ...
  rl.registerAction("webinar.get", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      return apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars/${(input as { webinarKey: string }).webinarKey}`);
    },
  });
  ...
  rl.registerAction("webinar.list", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      const data = (await apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars`)) as Record<string, unknown>;
      const list = (data._embedded as Record<string, unknown>)?.webinars ?? data;
      if ((input as Record<string, unknown>)?.limit && Array.isArray(list)) return (list as unknown[]).slice(0, (input as { limit: number }).limit);
      return list;
    },
  });
  ...
  rl.registerAction("webinar.update", {
    ...
    async execute(input, ctx) {
      const { webinarKey, subject, description: desc, times, timeZone } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
  ...
  rl.registerAction("webinar.delete", {
    ...
    async execute(input, ctx) {
      const { webinarKey, sendCancellationEmails } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
      const qs: Record<string, unknown> = {};
      if (sendCancellationEmails !== undefined) qs.sendCancellationEmails = sendCancellationEmails;
  ...
  rl.registerAction("registrant.create", {
    ...
    async execute(input, ctx) {
      const { webinarKey, firstName, lastName, email } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
  ...
  rl.registerAction("registrant.list", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      return apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars/${(input as { webinarKey: string }).webinarKey}/registrants`);
    },
  });
  ...
  rl.registerAction("session.list", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      return apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars/${(input as { webinarKey: string }).webinarKey}/sessions`);
    },
  });
  ...
  rl.registerAction("attendee.get", {
    ...
    async execute(input, ctx) {
      const { webinarKey, sessionKey, registrantKey } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
  ...
  rl.registerAction("coorganizer.create", {
    ...
    async execute(input, ctx) {
      const { webinarKey, external, organizerKey: coorgKey, givenName, email } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
  ...
  rl.registerAction("coorganizer.list", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      return apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars/${(input as { webinarKey: string }).webinarKey}/coorganizers`);
    },
  });
  ...
  rl.registerAction("coorganizer.delete", {
    ...
    async execute(input, ctx) {
      const { webinarKey, coorganizerKey, external } = input as Record<string, unknown>;
      const { accessToken, organizerKey } = getConn(ctx);
      const qs: Record<string, unknown> = {};
  ...
  rl.registerAction("panelist.list", {
    ...
    async execute(input, ctx) {
      const { accessToken, organizerKey } = getConn(ctx);
      return apiRequest(accessToken, "GET", `organizers/${organizerKey}/webinars/${(input as { webinarKey: string }).webinarKey}/panelists`);
    },
  });
