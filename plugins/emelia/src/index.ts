...
async function gql(apiKey: string, query: string, variables?: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (!res.ok) throw new Error(`Emelia API error ${res.status}: ${await res.text()}`);
  const data = (await res.json()) as Record<string, unknown>;
  if (data.errors) throw new Error(`Emelia GraphQL error: ${JSON.stringify(data.errors)}`);
...
export default function emelia(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("campaign.addContact", {
    ...
    async execute(input, ctx) {
      const { campaignId, email, firstName, lastName, customFields } = input as Record<string, unknown>;
  ...
  rl.registerAction("campaign.start", {
    ...
    inputSchema: { campaignId: { type: "string", required: true, description: "Campaign ID" } },
    async execute(input, ctx) {
      await gql(key(ctx), `mutation startCampaign($id: ID!) { startCampaign(id: $id) }`,
        { id: (input as { campaignId: string }).campaignId });
      return { success: true };
    },
  });
  ...
  rl.registerAction("campaign.pause", {
    ...
    inputSchema: { campaignId: { type: "string", required: true, description: "Campaign ID" } },
    async execute(input, ctx) {
      await gql(key(ctx), `mutation pauseCampaign($id: ID!) { pauseCampaign(id: $id) }`,
        { id: (input as { campaignId: string }).campaignId });
      return { success: true };
    },
  });
  ...
  rl.registerAction("campaign.duplicate", {
    ...
    async execute(input, ctx) {
      const { campaignId, name, copySettings = true, copyMails = true, copyContacts = false, copyProvider = true } =
  ...
  rl.registerAction("contactList.addContact", {
    ...
    async execute(input, ctx) {
      const { contactListId, email, firstName, lastName, customFields } = input as Record<string, unknown>;
      const contact: Record<string, unknown> = { email };
      if (firstName) contact.firstName = firstName;
      if (lastName) contact.lastName = lastName;
      if (customFields) Object.assign(contact, customFields);
