...
async function getAccessToken(
  ...
  if (!res.ok) throw new Error(`Bitwarden token error ${res.status}: ${await res.text()}`);
...
export default function bitwarden(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("collection.list", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await authedRequest(ctx, "GET", "/public/collections")) as { data: unknown[] };
  ...
  rl.registerAction("collection.update", {
    ...
    async execute(input, ctx) {
      const { collectionId, groups, externalId } = input as Record<string, unknown>;
      const body: Record<string, unknown> = {};
      if (groups) {
        body.groups = (groups as string[]).map((id) => ({ id, ReadOnly: false }));
      }
  ...
  rl.registerAction("event.list", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await authedRequest(ctx, "GET", "/public/events", undefined, qs)) as {
        data: unknown[];
      };
  ...
  rl.registerAction("group.create", {
    ...
    async execute(input, ctx) {
      const { name, accessAll, collections, externalId } = input as Record<string, unknown>;
      const body: Record<string, unknown> = { name, AccessAll: accessAll };
      if (collections) {
        body.collections = (collections as string[]).map((id) => ({ id, ReadOnly: false }));
      }
      if (externalId) body.externalId = externalId;
  ...
  rl.registerAction("group.list", {
    ...
    async execute(input, ctx) {
      const { limit } = (input ?? {}) as { limit?: number };
      const data = (await authedRequest(ctx, "GET", "/public/groups")) as { data: unknown[] };
      if (limit) return data.data.slice(0, limit);
      return data.data;
    },
  });
  ...
  rl.registerAction("group.getMembers", {
    ...
    async execute(input, ctx) {
      const { groupId } = input as { groupId: string };
      const memberIds = (await authedRequest(ctx, "GET", `/public/groups/${groupId}/member-ids`)) as string[];
  ...
  rl.registerAction("group.update", {
    ...
    async execute(input, ctx) {
      const { groupId, name, accessAll, collections, externalId } = input as Record<string, unknown>;
      const body: Record<string, unknown> = {};
      ...
      if (name) {
        ...
        const current = (await authedRequest(ctx, "GET", `/public/groups/${groupId}`)) as { name: string };
  ...
  rl.registerAction("group.updateMembers", {
    ...
    async execute(input, ctx) {
      const { groupId, memberIds } = input as { groupId: string; memberIds: string[] };
  ...
  rl.registerAction("member.create", {
    ...
    async execute(input, ctx) {
      const { email, type, accessAll, collections, externalId } = input as Record<string, unknown>;
      const body: Record<string, unknown> = { email, type, AccessAll: accessAll };
      if (collections) {
        body.collections = (collections as string[]).map((id) => ({ id, ReadOnly: false }));
      }
      if (externalId) body.externalId = externalId;
  ...
  rl.registerAction("member.list", {
    ...
    async execute(input, ctx) {
      const { limit } = (input ?? {}) as { limit?: number };
      const data = (await authedRequest(ctx, "GET", "/public/members")) as { data: unknown[] };
      if (limit) return data.data.slice(0, limit);
      return data.data;
    },
  });
  ...
  rl.registerAction("member.getGroups", {
    ...
    async execute(input, ctx) {
      const { memberId } = input as { memberId: string };
      const groupIds = (await authedRequest(ctx, "GET", `/public/members/${memberId}/group-ids`)) as string[];
  ...
  rl.registerAction("member.update", {
    ...
    async execute(input, ctx) {
      const { memberId, type, accessAll, collections, externalId } = input as Record<string, unknown>;
      const body: Record<string, unknown> = {};
  ...
  rl.registerAction("member.updateGroups", {
    ...
    async execute(input, ctx) {
      const { memberId, groupIds } = input as { memberId: string; groupIds: string[] };
