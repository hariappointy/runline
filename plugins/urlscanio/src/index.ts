...
async function apiRequest(
  ...
  if (!res.ok) throw new Error(`urlscan.io error ${res.status}: ${await res.text()}`);
  return res.json();
}
...
export default function urlscanio(rl: RunlinePluginAPI) {
  ...
  rl.registerAction("scan.get", {
    ...
    async execute(input, ctx) {
      return apiRequest(key(ctx), "GET", `/result/${(input as Record<string, unknown>).scanId}`);
    },
  });
  ...
  rl.registerAction("scan.search", {
    ...
    async execute(input, ctx) {
      ...
      const data = (await apiRequest(key(ctx), "GET", "/search", undefined, qs)) as Record<string, unknown>;
      return data.results;
    },
  });
}
